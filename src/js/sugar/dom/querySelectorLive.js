...
class SelectorLive {
	 ...
	 * Constructor
	 */
	constructor(selector, cb, settings = {}) {
...
export default function querySelectorLive(selector, cb = null, settings = {}) {
	...
	return Rx.Observable.create(observer => {
		...
		__domReady(() => {
			...
			// // create new instance of a selector live
			// const liveSelector = new SelectorLive(selector, cb, settings);
			//
			// // add the live selector into the stack
			// selectorsStack.push(liveSelector);
			//
			// // manage the observer
			// if ( ! settings.rootNode._sQueryLiveSelectorObserver) {
			// 	settings.rootNode._sQueryLiveSelectorObserver = new MutationObserver((mutations) => {
			// 		// loop on each
			// 	});
			// }
			...
			let added_callback = cb;
			if (cb instanceof Array) added_callback = cb[0];
